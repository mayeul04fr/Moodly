...
export default function StressScreen({ navigation }) {
    ...
    try {
      ...
      if (lastSubmission) {
        ...
        if (remainingTime > 0) {
          ...
          setTimeLeft(0);
          await AsyncStorage.removeItem('remainingTimeStress');
        }
      } else {
        setCanSubmitData(true);
        setTimeLeft(0);
  ...
  const storeSubmissionDate = async () => {
    try {
      ...
      await AsyncStorage.setItem('lastSubmissionDateStress', currentDate);
    } catch (error) {
      ...
      <View style={{ alignItems: 'center', gap: 50 }}>
        ...
        <View style={{ gap: 32, width: '90%' }}>
          ...
          >
            ...
            <Text style={{ color: '#000' }}>{stressPhrases[value - 1]}</Text>
          </Text>
