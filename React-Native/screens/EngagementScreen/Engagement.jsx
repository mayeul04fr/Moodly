...
export default function EngagementScreen({ navigation }) {
    ...
    try {
      ...
      if (lastSubmission) {
        ...
        if (remainingTime > 0) {
          ...
          setTimeLeft(0);
          await AsyncStorage.removeItem('remainingTimeEngagement');
        }
      } else {
        setCanSubmitData(true);
        setTimeLeft(0);
  ...
  const storeSubmissionDate = async () => {
    try {
      ...
      await AsyncStorage.setItem('lastSubmissionDateEngagement', currentDate);
    } catch (error) {
